...
public class Path {
    public static void main(String args[]) throws Exception {
        ...
        // expected output: "(parent: null distance: 2147483647 destination: A)(parent:
        // null distance: 0 destination: B)(parent: null distance: 2147483647
        // destination: C)(parent: null distance: 2147483647 destination: D)(parent:
        // null distance: 2147483647 destination: E)(parent: null distance: 2147483647
        // destination: F)(parent: null distance: 2147483647 destination: G)"
        ...
        // expected output: "(parent: F distance: 5 destination: A)(parent: null
        // distance: 0 destination: B)(parent: G distance: 8 destination: C)(parent: C
        // distance: 9 destination: D)(parent: G distance: 9 destination: E)(parent: B
        // distance: 2 destination: F)(parent: A distance: 6 destination: G)"
    ...
    public static List<PathVertex> dijkstra(ExtendedGraph g, Vertex v) {
        ...
        // used for extracting minimum. This gets one element smaller after each
        // iteration (see below)
        // ...
        ...
        // since the vertex (incidentVertex of type Vertex) returned from incident
        // vertex
        // does not hold information about its current distance to the source,
        // we look for its exact counterpart in paths (incidentPathVertex of type
        // PathVertex):
        // for...
        // ...
        // ...
        ...
        PriorityQueue<PathVertex> pq = updatePriorityQueueDistances(paths);

        while (!pq.isEmpty()) {
            PathVertex u = pq.poll();
            u.visited = true;

            List<Edge> incidentEdges = g.incidentEdges(u);
            for (Edge edge : incidentEdges) {
                Vertex incidentVertex = edge.vertex2;
                PathVertex incidentPathVertex = null;
                for (PathVertex pathVertex : paths) {
                    if (pathVertex.vertex.getLabel().equals(incidentVertex.getLabel())) {
                ...
                if (incidentPathVertex != null) {
                    if (relaxEdge(u, incidentPathVertex, edge.weight)) {
                        // Update the priority queue with the updated distance
                        pq.remove(incidentPathVertex);
                        pq.offer(incidentPathVertex);
                    }
                }
            }
        }
    ...
    /**
     * Initializes the single source for Dijkstra's algorithm.
     *
     * @param g the graph
     * @param s the source vertex
     * @return the list of path vertices with distances initialized
     */
    ...
    // ** TO DO **
    public static List<PathVertex> initializeSingleSource(ExtendedGraph g, Vertex s) {
        // This sets every PathVertex's parent to null and its distance to the source
        // infinity
        // except for the source (s) where its distance is 0
        // get the vertices (which is of type Vertex) from g and create a List of type
        // PathVertex
        List<PathVertex> paths = new ArrayList<>();
        for (Vertex vertex : g.vertices()) {
            PathVertex pathVertex = new PathVertex(vertex.getLabel());
